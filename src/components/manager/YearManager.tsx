...
const YearManager: React.FC = () => {
  ...
  const handleSetActive = async (id: string) => {
    try {
      ...
      // Update local state: set the targeted year as active and all others as inactive
      setYears(
        years.map((year) => ({
          ...year,
          active: year._id === id,
      ...
      setNotification({
        type: "error",
        message: error.message || "Failed to set active year",
      });
    } finally {
      setIsLoading(false);
      setTimeout(() => setNotification(null), 3000);
  ...
  const handleExportExcel = () => {
    ...
    link.setAttribute(
      "download",
      `academic_years_${new Date().toISOString().split("T")[0]}.csv`
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 bg-gradient-to-r from-indigo-700 to-purple-700 text-white p-4 rounded-lg shadow-md">
        <div>
          <h2 className="text-xl font-bold">Year/Level Management</h2>
          <p className="text-indigo-100 text-sm font-sans font-light">
            Manage school years and student levels for elections
          </p>
        </div>
      ...
      {!isLoading && years.length === 0 && (
        <div className="bg-white rounded-lg p-8 text-center">
          ...
          <p className="text-gray-500 mb-6">
            Get started by adding academic years using the "Add Year" button
            above
          </p>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Only one year can be active at a time. Setting this as active
                will deactivate all other years.
              </p>
            </div>
          </div>
